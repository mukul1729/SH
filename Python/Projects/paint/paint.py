...
class Paint:
    def __init__(self):
        ...
        self.projectDir = \
                '/home/mukul/Documents/SH/Python/Projects/paint/'
        self.val = 1
        self.var = None
        self.r = 0
        self.g = 0
    ...
    def showval(self,setting_Surf,var,pos,s):
        fontObj = \
            pygame.font.Font('/home/mukul/Downloads/OpenSans-Regular.ttf',18)
    ...
    def queue_floodfill(self, arr, x, y, val, preval):
        """ graph algorithm for filling colors inside of shapes
        """
        random_queue = []
        random_queue.append((x, y))
        while(random_queue):
            x, y = random_queue.pop()
            curr_pixel = arr[x][y]
            if(x < 0 or y < 0 or x > (currx-1) or y > (curry-1) or
                curr_pixel != list(preval[0:-1])):
                continue

            arr[x][y] = val
            random_queue.extend(((x, y-1), (x+1, y), (x-1, y), (x, y+1)))

    def tabar(self,scr,x,y,position,mouse,mouseButton,projectDir,var):
        tab = pygame.image.load(projectDir + '/images/tab.png').convert()
        scr.blit(tab,(x,y))
        if var == 1:
            tab = pygame.image.load(projectDir +
                    '/images/tab_sel.png').convert()
            scr.blit(tab,(x,y))
        cross = pygame.image.load(projectDir + '/images/cross.png').convert()
        scr.blit(cross,(x+128,y))
        if position[0] > x+128 and position[0]<x+153 and position[1]>y and\
                position[1]<y+25:
            cross = pygame.image.load(projectDir +
                    '/images/cross_h.png').convert()
            scr.blit(cross,(x+128,y))
            if mouseButton == 'left':
                time.sleep(.10)
            ...
            if var == 1:
                tab = pygame.image.load(projectDir +
                        '/images/tab_sel_h.png').convert()
            else:
                tab = pygame.image.load(projectDir +
                        '/images/tab_h.png').convert()
            scr.blit(tab,(x,y))
            if mouseButton == 'left':
                if var == 1:
                    tab = pygame.image.load(projectDir +
    ...
    def plus(self,scr,x,y,position,mouse,mouseButton,projectDir):
        add = pygame.image.load(projectDir + '/images/add.png').convert()
        scr.blit(add,(x,y))
        if position[0] > x and position[0]<x+25 and position[1]>y and\
                position[1]<y+25:
            add = pygame.image.load(projectDir + '/images/add_h.png').convert()
            scr.blit(add,(x,y))
            if mouseButton == 'left':
                time.sleep(0.10)
                if self.tab in range(0,5):
                    self.tab += 1
                add = pygame.image.load(projectDir +
                        '/images/add.png').convert()
                scr.blit(add,(x,y))
    ...
    def scrollbar(self,scr,x,y,position,mouse,mouseButton,var,color):
        fontObj = \
            pygame.font.Font('/home/mukul/Downloads/OpenSans-Regular.ttf',20)
    ...
    def settings_ui(self,setting_Surf,position,mouse,mouseButton,scr):
        ...
        if self.sett == 0:
            ...
            pygame.draw.line(setting_Surf,(0,0,0),(182,0),(182,287),6)
            a1 = self.scrollbar(setting_Surf,30,40,position,mouse,\
                    mouseButton,self.scx1,color)
            self.size = a1 - 30
            self.scx1 = a1
            self.showval(setting_Surf,a1-30,(60,60),'Value: ')
            b1 = self.scrollbar(setting_Surf,30,120,position,mouse,\
                    mouseButton,self.scx2,color)
            self.gx = 879 - (5*(b1-31))
            self.scx2 = b1
            self.showval(setting_Surf,self.gx,(50,140),'Value: ')
            c1 = self.scrollbar(setting_Surf,30,210,position,mouse,\
                    mouseButton,self.scx3,color)
            self.gy = 590 - (3*(c1-31))
            self.scx3 = c1
            ...
            pygame.draw.line(setting_Surf,(0,0,0),(182,0),(182,287),6)
            a = self.scrollbar(setting_Surf,30,40,position,mouse,\
                    mouseButton,self.scx4,color)
            self.r = 2.15*(a - 30)
            self.scx4 = a
            b = self.scrollbar(setting_Surf,30,120,position,mouse,\
                    mouseButton,self.scx5,color)
            self.g = 2.15*(b - 30)
            self.scx5 = b
            c = self.scrollbar(setting_Surf,30,210,position,mouse,\
                    mouseButton,self.scx6,color)
            self.b = 2.15*(c - 30)
            self.scx6 = c
    ...
    def ui(self,scr,position,mouse,mouseButton):
        ...
        fontObj = \
            pygame.font.Font('/home/mukul/Downloads/OpenSans-Regular.ttf',18)
        fontObj2 = \
            pygame.font.Font('/home/mukul/Downloads/OpenSans-Regular.ttf',15)
        ...
        text4 = fontObj.render('Eraser',True,(90,90,90))
        ...
        box = pygame.image.load(projectDir + '/images/box.png').convert()
        save = pygame.image.load(projectDir+ '/images/save.png').convert()
        tabular = pygame.image.load(projectDir +
                '/images/tabular.png').convert()
        ...
        for a in range(0,self.tab):
            if self.tabarr[self.val] == self.tabarr[self.tabseq[a]]:
                ...
                self.var = 0
            self.tabar(scr,self.tabarr[self.tabseq[a]],93,position,\
                    mouse,mouseButton,projectDir,self.var)
        if self.tab <=0:
            self.val = self.tabseq[0]
        ...
        if mouse == 'motion':
            if tool == 'rectangle':
                box = pygame.image.load(projectDir +
                        '/images/box_hover.png').convert()
                scr.blit(box,(20,10))
                scr.blit(text1,(44,25))
            elif tool == 'line':
                box = pygame.image.load(projectDir +
                ...
                save = pygame.image.load(projectDir +
                        '/images/save_h.png').convert()
                scr.blit(save,(1000,10))
                scr.blit(text8,(1020,13))
            elif tool == 'open':
                save = pygame.image.load(projectDir +
        ...
        selected = fontObj.render('Tool: '+str(self.selected),True,\
                (200,200,200))
...
flags = DOUBLEBUF
screen = pygame.display.set_mode((obj_paint.x,obj_paint.y), flags, 16)
...
while obj_paint.run == True:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            root.filename =  filedialog.askopenfilename(initialdir =\
                    ...
                    (("jpeg files","*.jpg"),("all files","*.*")))
            if root.filename:
                img = pygame.image.load(root.filename).convert()
            ...
            if obj_paint.selected == 'freehand':
                if mouseButton == 'left':
                    if c==0:
                        ...
                        c=1
                    pygame.draw.line(game_Surf,obj_paint.sel_color,\
                            (x1-194,y1-124),(x2-194,y2-124))
                    x1,y1 = x2,y2
                    x2,y2 = pos_motion[0],pos_motion[1]
                    screen.blit(game_Surf,(194,124))
                ...
                if mouseButton == 'left':
                    ...
                    if cpy:
                        game_Surf.blit(cpy,(0,0))
                    pygame.draw.line(game_Surf,obj_paint.sel_color,\
                            (x1-194,y1-124),(x2-194,y2-124),obj_paint.size)
                    screen.blit(game_Surf,(194,124))
                ...
                if mouseButton == 'left':
                    ...
                    if cpy:
                        game_Surf.blit(cpy,(0,0))
                    try:
                        pygame.draw.rect(game_Surf,obj_paint.sel_color,\
                                (x1-194,y1-124,x2-x1,y2-y1),1)
                    except ValueError:
                        pass
                    screen.blit(game_Surf,(194,124))
                    ...
                    try:
                        ...
                        if x1<x2:
                            for i in range(x1,x2,3):
                                pygame.draw.line(game_Surf,(255,255,255),\
                                        (i-194,y1-124),((i-194)+1,y1-124),3)
                                pygame.draw.line(game_Surf,(255,255,255),\
                                        (i-194,y2-124),((i-194)+1,y2-124),3)
                        else:
                        ...
                        if y1<y2:
                            for i in range(y1,y2,3):
                                pygame.draw.line(game_Surf,(255,255,255),\
                                        (x1-194,i-124),(x1-194,i-124+1),3)
                                pygame.draw.line(game_Surf,(255,255,255),\
                                        (x2-194,i-124),(x2-194,i-124+1),3)
                        else:
                ...
                screen.blit(copy_surf,(pos_motion[0]-(x2-x1)//2,\
                        pos_motion[1]-(y2-y1)//2))
                ...
                if mouseButton == 'left':
                    ...
                    rady = max(y2-y1,1)
                    sz2 = obj_paint.size
                    pygame.draw.ellipse(game_Surf,obj_paint.sel_color,(x1-194,\
                        y1-124,radx,rady), sz2 if sz2 < max(radx, rady) else 0)
                    screen.blit(game_Surf,(194,124))

            elif obj_paint.selected == 'circle':
                if mouseButton == 'left':
                    if c==0:
                        x1,y1 = pos_down[0],pos_down[1]
                        x2,y2 = pos_motion[0],pos_motion[1]
                        c=1
                    pygame.draw.rect(game_Surf,(255,255,255),(x1-194,y1-124,\
                            25,25),0)
                    x1,y1 = x2,y2
                    x2,y2 = pos_motion[0],pos_motion[1]
                    screen.blit(game_Surf,(194,124))
            ...
            if obj_paint.selected == 'bucket' and pos_down[0]>192 and\
                ...
                obj_paint.queue_floodfill(arr3d,x+1,y,obj_paint.sel_color,\
            ...
            if obj_paint.selected == 'copy':
                ...
                y2 in range(124,721):
                    ...
                    copy_surf.blit(copy,(0,0),pygame.Rect(x1-194,y1-124,x2-194\
                        ,y2-124))
    ...
    if obj_paint.selected == 'copy_movin':
        screen.blit(copy_surf,(pos_motion[0]-(x2-x1)//2,pos_motion[1]-\
                (y2-y1)//2))
        if mouse == 'down':
